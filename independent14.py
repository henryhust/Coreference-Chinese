...
class CorefModel(object):
  def __init__(self, config):
    ...
    input_props.append((tf.int32, [None, None]))    # segments_ids.
  ...
  def tensorize_example(self, example, is_training):
    ...
    #TODO:按句子进行切分
    sentences_flatten = util.flatten(sentences)
    sentencemaps_num = len(set(sentence_map))
    segments = []         # 句子ids
    segments_len = []     # 句子长度
    max_segments_len = 0
    for i in range(sentencemaps_num):
        segment_char_ids = []
        for idx, j in enumerate(sentence_map):
            if i == j:
                segment_char_ids.append(self.char_dict.get(sentences_flatten[idx], 0))
            elif i < j:
                continue
            else:
              break
        segments.append(segment_char_ids)
        segments_len.append(len(segment_char_ids))
        max_segments_len = len(segment_char_ids) if len(segment_char_ids) > max_segments_len else max_segments_len

    segments_ids = []
    for seg in segments:
        while len(seg) < max_segments_len:
            seg.append(0)
        segments_ids.append(seg)
    segments_ids = np.array(segments_ids)
    segments_len = np.array(segments_len)
    ...
    example_tensors = (input_ids, char_ids, segments_ids, segments_len, input_mask,  text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map)
  ...
  def truncate_example(self, input_ids, char_ids, segments_ids, segments_len, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, sentence_offset=None):
    ...
    return input_ids, char_ids, segments_ids, segments_len, input_mask, text_len, speaker_ids, postag_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map
  ...
  def get_predictions_and_loss(self, input_ids, char_ids, segments_ids, segments_len, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    char_emb = tf.get_variable("char_embedding", [len(self.char_dict), self.config["feature_size"]],
    ...
    segments_char_emb = tf.gather(char_emb, segments_ids)

    bilstm_out = util.biGRU_layer(segments_char_emb, 20, segments_len)
    sentence_emb = util.attention(bilstm_out, 20)
    ...
    candidate_char_emb = tf.gather(char_emb, candidate_char_ids)
    ...
    top_sentence_emb = tf.gather(sentence_emb, top_sentence_indices)
    # 字向量与句子向量拼接
  ...
  def evaluate(self, session, global_step=None, official_stdout=False, mod="eval", keys=None, eval_mode=False):
    ...
    for example_num, (tensorized_example, example) in enumerate(data):
      _, _, _, _, _, _, _, _, _, _, gold_starts, gold_ends, _, _ = tensorized_example
