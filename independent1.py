...
class CorefModel(object):
  ...
  def get_postag_emb(self, pos_outputs, span_starts, span_ends):
      """获取syntatic head特征向量"""
      mention_pos_scores = self.get_masked_mention_word_scores(pos_outputs, span_starts, span_ends, mod="pos")  # 获取attention系数

      # 矩阵乘法，实现span范围内，wordemb的加权乘积
      head_attn_reps = tf.matmul(mention_pos_scores, pos_outputs)  # [K, emb]
      return head_attn_reps
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    # 获取postag_att特征表示
    candidate_att_pos_emb = self.get_postag_emb(postag_doc, candidate_starts, candidate_ends)

    candidate_mention_scores = self.get_mention_scores(candidate_span_emb, candidate_starts, candidate_ends)
    candidate_mention_pos_scores = self.get_pos_scores(candidate_att_pos_emb, candidate_starts, candidate_ends)
    
    candidate_mention_scores += candidate_mention_pos_scores
  ...
  def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends):
    ...
    if self.config["model_heads"]:
      """获取mentionscore"""
      mention_word_scores = self.get_masked_mention_word_scores(context_outputs, span_starts, span_ends, mod="word")    # 获取attention系数
  ...
  def get_pos_scores(self, pos_emb, span_starts, span_ends):
    with tf.variable_scope("mention_pos_scores"):
      pos_scores = util.ffnn(pos_emb, self.config["ffnn_depth"], self.config["ffnn_size"], 1, self.dropout) # [k, 1]
    return pos_scores
  ...
  def get_masked_mention_word_scores(self, encoded_doc, span_starts, span_ends, mod="word"):
      ...
      if mod == "word":
        ...
        with tf.variable_scope("mention_pos_attn"):
          word_attn = tf.squeeze(util.projection(encoded_doc, 1, initializer=tf.truncated_normal_initializer(stddev=0.02)), 1)
