...
class CorefModel(object):
  def __init__(self, config):
    ...
    self.char_dict = util.load_char_dict(config["char_vocab"])
    ...
    input_props.append((tf.int32, [None, None]))    # char_ids.
    ...
    self.input_tensors = queue.dequeue()                        # 队列获取数据

    self.predictions, self.loss = self.get_predictions_and_loss(*self.input_tensors)      # 模型部分
    # bert stuff
    ...
    init_from_checkpoint = tf.train.init_from_checkpoint if config['init_checkpoint'].endswith('ckpt') else None
      ...
      while True:
        ...
        if self.config['single_example']:
          for example in train_examples:
            tensorized_example = self.tensorize_example(example, is_training=True)      # 处理数据
  ...
  def tensorize_example(self, example, is_training):
    ...
    gold_mentions = sorted(tuple(m) for m in util.flatten(clusters))      # gold_mention 展开排序
    ...
    max_sentence_length = self.max_segment_len                            # 最大句长，128
    ...
    for i, (sentence, speaker) in enumerate(zip(sentences, speakers)):
        ...
        sent_char_index = [self.char_dict.get(c, 0) for c in sentence]
        ...
        while len(sent_input_ids) < max_sentence_length:
            ...
            sent_char_index.append(0)
        input_ids.append(sent_input_ids)
        speaker_ids.append(sent_speaker_ids)
        input_mask.append(sent_input_mask)
        char_ids.append(sent_char_index)
    input_ids = np.array(input_ids)                                         # sentence转化为ids
    ...
    example_tensors = (input_ids, char_ids, input_mask,  text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map)

    if is_training and len(sentences) > self.config["max_training_sentences"]:      # 大于11
      if self.config['single_example']:
  ...
  def truncate_example(self, input_ids, char_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, sentence_offset=None):
    """对句子个数进行限制, [bert_base]max_training_sentences = 11"""
    ...
    char_ids = char_ids[sentence_offset:sentence_offset + max_training_sentences, :]
    ...
    return input_ids, char_ids, input_mask, text_len, speaker_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map
  ...
  def coarse_to_fine_pruning(self, top_span_emb, top_span_mention_scores, c):
    """
    计算mention pairs的得分
    :param top_span_emb: mention的特征表示
    :param top_span_mention_scores: mention的得分
    :param c: 最大的候选antecedent个数
    :return:
    """
  ...
  def get_predictions_and_loss(self, input_ids, char_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    # 三维[seq_len, max_span_len, max_span_len]
    candidate_char = tf.add(tf.tile(tf.expand_dims(candidate_starts, axis=2), [1, 1, self.max_span_width]), tf.range(self.max_span_width))
    # 可能的span获取char_emb的掩码
    # candidate_mask_char = tf.tile(tf.expand_dims(candidate_mask, axis=2))
    candidate_char_ends = tf.tile(tf.expand_dims(candidate_ends, axis=2), [1, 1, self.max_span_width])
    candidate_char_mask = tf.logical_and(True, candidate_char < candidate_char_ends)
    candidate_char = tf.to_float(candidate_char) + tf.log(tf.to_float(candidate_char_mask))

    candidate_char_index = tf.where(candidate_char < 0, tf.zeros_like(candidate_char), candidate_char)    # [seq, max_span_width, max_span_width]
    candidate_char_ids = tf.gather(flatten_char_ids, tf.to_int32(candidate_char_index), axis=0)
    candidate_char_emb = tf.gather(tf.get_variable("char_embedding", [len(self.char_dict), self.config["feature_size"]],
                                         initializer=tf.truncated_normal_initializer(stddev=0.02)), candidate_char_ids)
    candidate_char_emb = tf.reshape(candidate_char_emb, [-1, self.max_span_width, self.config["feature_size"]])
    # [seq*max_span_width, len(filter)*num_filters=30]
    candidate_span_cnn = util.cnn(tf.to_float(candidate_char_emb), filter_sizes=[2, 3, 4], num_filters=10)

    # flattened_candidate_char_mask = tf.reshape(candidate_char_mask, [num_words * self.max_span_width, -1])
    # candidate_char = tf.boolean_mask(tf.reshape(candidate_char, [num_words * self.max_span_width, -1]), flattened_candidate_char_mask)
    ...
    # 二维特征，一维掩码
    candidate_span_cnn = tf.boolean_mask(candidate_span_cnn, flattened_candidate_mask)

    candidate_sentence_indices = tf.boolean_mask(tf.reshape(candidate_start_sentence_indices, [-1]), flattened_candidate_mask) # [num_candidates]

    candidate_cluster_ids = self.get_candidate_labels(candidate_starts, candidate_ends, gold_starts, gold_ends, cluster_ids) # [num_candidates]

    # candidate_span_emb = 头 + 尾 + width + attention
    candidate_span_emb = self.get_span_emb(candidate_span_cnn, mention_doc, candidate_starts, candidate_ends) # [num_candidates, emb]
    ...
    if self.config['use_pos']:
      pos_ids = self.flatten_emb_by_sentence()
    ...
    word_segments = tf.tile(tf.expand_dims(tf.range(0, num_segs), 1), [1, seg_len])     # N , L
    ...
    if self.config['fine_grained']:
        for i in range(self.config["coref_depth"]):     # coref_depth = 2
            with tf.variable_scope("coref_layer", reuse=(i > 0)):
                top_antecedent_emb = tf.gather(top_span_emb, top_antecedents)  # [k, c, emb], 从 top_span 中提取 top_antecedents
                top_antecedent_scores = top_fast_antecedent_scores +\
                                        self.get_slow_antecedent_scores(top_span_emb, top_antecedents, top_antecedent_emb, top_antecedent_offsets, top_span_speaker_ids, genre_emb, segment_distance) # [k, c]
                # antecedent的权重， 再 * top_antecedent_emb
  ...
  def get_span_emb(self, span_cnn_emb, context_outputs, span_starts, span_ends):
    span_emb_list = []
    ...
    # 添加span_cnn_emb特征
    span_emb_list.append(span_cnn_emb)
    ...
    #   """获取mentionscore"""
    #   mention_word_scores = self.get_masked_mention_word_scores(context_outputs, span_starts, span_ends)    # 获取attention系数
    #   head_attn_reps = tf.matmul(mention_word_scores, context_outputs)          # [K, emb]
  ...
  def bucket_distance(self, distances):
    ...
    """
    logspace_idx = tf.to_int32(tf.floor(tf.log(tf.to_float(distances))/math.log(2))) + 3      # 以2为底，distances的对数
  ...
  def evaluate_coref(self, top_span_starts, top_span_ends, predicted_antecedents, gold_clusters, evaluator):
    ...
    return predicted_clusters, mention_to_predicted, mention_to_gold

  def load_eval_data(self, mod):
      def load_line(line):
          example = json.loads(line)
          return self.tensorize_example(example, is_training=False), example
      if mod == "eval" and self.eval_data is None:
          with open(self.config["eval_path"]) as f:
              self.eval_data = [load_line(l) for l in f.readlines()]
          num_words = sum(tensorized_example[2].sum() for tensorized_example, _ in self.eval_data)
          print("Loaded {} eval examples.".format(len(self.eval_data)))
      elif mod == "test" and self.test_data is None:
          with open(self.config["test_path"]) as f:
              self.test_data = [load_line(l) for l in f.readlines()]
          print("Load {} test examples.".format(len(self.test_data)))

  def evaluate(self, session, global_step=None, official_stdout=False, mod="eval", keys=None, eval_mode=False):
    self.load_eval_data(mod)
    ...
    for example_num, (tensorized_example, example) in enumerate(data):
      ...
      coref_predictions[example["doc_key"]], mention_to_predicted, mention_to_gold = self.evaluate_coref(top_span_starts, top_span_ends, predicted_antecedents, example["clusters"], coref_evaluator)

      for pre in mention_to_predicted.keys():
        if pre in mention_to_gold.keys():
          mention_pre += 1
      mention_gold += len(mention_to_gold.keys())

      if example_num % 10 == 0:
          print("{}-process:{}/{} examples.".format(mod, example_num + 1, len(data)))

    summary_dict = {}


    p, r, f = coref_evaluator.get_prf()
    men_r = (mention_pre/mention_gold)
    if mod == "eval":
        summary_dict["(eval)Average Mention Recall"] = men_r
        summary_dict["(eval)Average F1 (py)"] = f
        summary_dict["(eval)Average precision (py)"] = p
        summary_dict["(eval)Average recall (py)"] = r
    if mod == "test":
        summary_dict["(test)Average Mention Recall"] = men_r
        summary_dict["(test)Average F1 (py)"] = f
        summary_dict["(test)Average precision (py)"] = p
        summary_dict["(test)Average recall (py)"] = r
        
    return util.make_summary(summary_dict), f, p, r, men_r


class CorefModel_pos(object):
    """对指代消解模型进行修改，添加pos信息"""
    def __init__(self, config):
      self.config = config
      self.max_segment_len = config['max_segment_len']
      ...
      input_props.append((tf.int32, [None]))          # Text lengths.
      input_props.append((tf.int32, [None, None]))    # Speaker IDs.
      input_props.append((tf.int32, [None, None]))    # Postags
      input_props.append((tf.int32, []))              # Genre.
      input_props.append((tf.bool, []))               # Is training.
      input_props.append((tf.int32, [None]))          # Gold starts.
      input_props.append((tf.int32, [None]))          # Gold ends.
      ...
      self.queue_input_tensors = [tf.placeholder(dtype, shape) for dtype, shape in input_props]
      dtypes, shapes = zip(*input_props)
      queue = tf.PaddingFIFOQueue(capacity=5, dtypes=dtypes, shapes=shapes)   # 声明一个带有padding功能的队列

      self.enqueue_op = queue.enqueue(self.queue_input_tensors)               # 先构建图，尚未进行计算
      self.input_tensors = queue.dequeue()                                    # 队列获取数据
    ...
    def start_enqueue_thread(self, session):
      ...
      def _enqueue_loop():
        while True:
          random.shuffle(train_examples)
          if self.config['single_example']:
            for example in train_examples:      # 获取训练数据
              tensorized_example = self.tensorize_example(example, is_training=True)      # 处理数据
              feed_dict = dict(zip(self.queue_input_tensors, tensorized_example))         # place_holder:数据
              session.run(self.enqueue_op, feed_dict=feed_dict)                           #
          else:
            examples = []
            for example in train_examples:      # 获取训练数据
    ...
    def tensorize_example(self, example, is_training):
        """从数据example中提取输入数据, 添加postag数据"""
        clusters = example["clusters"]

        gold_mentions = sorted(tuple(m) for m in util.flatten(clusters))      # gold_mention
        gold_mention_map = {m: i for i, m in enumerate(gold_mentions)}        # mention_map
        cluster_ids = np.zeros(len(gold_mentions))                            # cluster_ids
        for cluster_id, cluster in enumerate(clusters):
          for mention in cluster:
            cluster_ids[gold_mention_map[tuple(mention)]] = cluster_id + 1
        ...
        sentences = example["sentences"]
        num_words = sum(len(s) for s in sentences)
        speakers = example["speakers"]                                        # speakers
        postags = example["postags"]                                          # postags

        # assert num_words == len(speakers), (num_words, len(speakers))
        speaker_dict = self.get_speaker_dict(util.flatten(speakers))
        self.postags_dict = self.get_pos_dict(util.flatten(postags))

        sentence_map = example['sentence_map']                                # sentence_map

        max_sentence_length = self.max_segment_len
        text_len = np.array([len(s) for s in sentences])

        input_ids, input_mask, speaker_ids, postag_ids = [], [], [], []
        for i, (sentence, speaker, postag) in enumerate(zip(sentences, speakers, postags)):
            sent_input_ids = self.tokenizer.convert_tokens_to_ids(sentence)
            sent_input_mask = [1] * len(sent_input_ids)
            sent_speaker_ids = [speaker_dict.get(s, 3) for s in speaker]
            sent_postag_ids = [self.postags_dict.get(p, 1) for p in postag]
            ...
            while len(sent_input_ids) < max_sentence_length:
                ...
                sent_input_mask.append(0)
                sent_speaker_ids.append(0)
                sent_postag_ids.append(0)
            input_ids.append(sent_input_ids)
            speaker_ids.append(sent_speaker_ids)
            postag_ids.append(sent_postag_ids)
            input_mask.append(sent_input_mask)
        input_ids = np.array(input_ids)
        input_mask = np.array(input_mask)
        speaker_ids = np.array(speaker_ids)
        postag_ids = np.array(postag_ids)
        assert num_words == np.sum(input_mask), (num_words, np.sum(input_mask))
        ...
        gold_starts, gold_ends = self.tensorize_mentions(gold_mentions)
        example_tensors = (input_ids, input_mask,  text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map)
    ...
    def truncate_example(self, input_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, sentence_offset=None):
      """对句子个数进行限制, [bert_base]max_training_sentences = 11"""
      max_training_sentences = self.config["max_training_sentences"]
      num_sentences = input_ids.shape[0]
      assert num_sentences > max_training_sentences
      ...
      input_mask = input_mask[sentence_offset:sentence_offset + max_training_sentences, :]
      speaker_ids = speaker_ids[sentence_offset:sentence_offset + max_training_sentences, :]
      postag_ids = postag_ids[sentence_offset:sentence_offset + max_training_sentences, :]
      text_len = text_len[sentence_offset:sentence_offset + max_training_sentences]
      ...
      return input_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
      ...
      if self.config['use_pos']:
        postag_ids = self.flatten_emb_by_sentence(postag_ids, input_mask)
        top_span_postag_ids = tf.gather(postag_ids, top_span_starts)
      else:
        top_span_postag_ids = None
      ...
      if self.config['fine_grained']:
        for i in range(self.config["coref_depth"]):
          with tf.variable_scope("coref_layer", reuse=(i > 0)):
            ...
            top_antecedent_scores = top_fast_antecedent_scores + self.get_slow_antecedent_scores(top_span_emb, top_antecedents, top_antecedent_emb, top_antecedent_offsets, top_span_speaker_ids, top_span_postag_ids, genre_emb, segment_distance) # [k, c]
            top_antecedent_weights = tf.nn.softmax(tf.concat([dummy_scores, top_antecedent_scores], 1)) # [k, c + 1]
            top_antecedent_emb = tf.concat([tf.expand_dims(top_span_emb, 1), top_antecedent_emb], 1) # [k, c + 1, emb]
            attended_span_emb = tf.reduce_sum(tf.expand_dims(top_antecedent_weights, 2) * top_antecedent_emb, 1) # [k, emb]
            with tf.variable_scope("f"):
    ...
    def get_slow_antecedent_scores(self, top_span_emb, top_antecedents, top_antecedent_emb, top_antecedent_offsets, top_span_speaker_ids, top_span_postag_ids, genre_emb, segment_distance=None):
      k = util.shape(top_span_emb, 0)
      c = util.shape(top_antecedents, 1)
      ...
      if self.config["use_pos"]:
        top_antecedent_postag_ids = tf.gather(top_span_postag_ids, top_antecedents)  # [k, c]
        antecedent_postag_emb = tf.gather(
          tf.get_variable("postag_emb",
                          [29, self.config["feature_size"]],
                          initializer=tf.truncated_normal_initializer(stddev=0.02)
                          ),
          tf.to_int32(top_antecedent_postag_ids)
        )  # [k, c, emb]
        feature_emb_list.append(antecedent_postag_emb)
      ...
      # span i 与 antecendent 之间的关联， span i + span j + similarity + feature
      pair_emb = tf.concat([target_emb, top_antecedent_emb, similarity_emb, feature_emb], 2) # [k, c, emb]
    ...
    def load_eval_data(self):
      if self.eval_data is None:
        def load_line(line):
          example = json.loads(line)              # 下一句为获取训练数据
          return self.tensorize_example(example, is_training=False), example
        with open(self.config["eval_path"]) as f:
          self.eval_data = [load_line(l) for l in f.readlines()]
        num_words = sum(tensorized_example[2].sum() for tensorized_example, _ in self.eval_data)
    ...
    def evaluate(self, session, global_step=None, official_stdout=False, keys=None, eval_mode=False):
      ...
      p,r,f = coref_evaluator.get_prf()
      men_r = (mention_pre/mention_gold)
      summary_dict["Average Mention Recall"] = men_r
      print("Mention Recall = {:.2f}%".format(men_r * 100))
