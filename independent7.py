...
class CorefModel(object):
  def __init__(self, config):
    self.config = config
    self.word_emb = util.EmbeddingDictionary(config["word_emb_path"])

    # with tf.device('/cpu:0'), tf.name_scope("embedding"):
    self.W = tf.get_variable("word_embeddings", initializer=tf.to_float(self.word_emb._embedding_param), trainable=True, dtype=tf.float32)
  ...
  def tensorize_example(self, example, is_training):
    ...
    for i, (sentence, speaker) in enumerate(zip(sentences, speakers)):
        ...
        sent_input_ids = self.word_emb.convert_tokens_to_ids(sentence)
        sent_input_mask = [1] * len(sentence)
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    # 预训练词嵌入查表
    # emb = self.word_emb._embedding_param
    self.embedded_chars = tf.nn.embedding_lookup(self.W, input_ids)
    bilstm_output = util.biLSTM_layer(self.embedded_chars, lstm_dim=256, lengths=text_len, name="bilstm-1")
    bilstm_output = util.biLSTM_layer(bilstm_output, lstm_dim=256, lengths=text_len, name="bilstm-2")
    
    self.dropout = self.get_dropout(self.config["dropout_rate"], is_training)
    mention_doc = tf.nn.dropout(bilstm_output, self.dropout)
