...
class CorefModel(object):
  def __init__(self, config):
    ...
    input_props.append((tf.int32, [None, None]))    # Postag IDs.
  ...
  def tensorize_example(self, example, is_training):
    ...
    for i, (sentence, speaker, postag) in enumerate(zip(sentences, speakers, postags)):
        ...
        sent_postag_ids = [self.postag.get(p, 0) for p in postag]
    ...
    example_tensors = (input_ids, char_ids, input_mask,  text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map)
  ...
  def truncate_example(self, input_ids, char_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, sentence_offset=None):
    ...
    return input_ids, char_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map
  ...
  def coarse_to_fine_pruning(self, top_span_emb, top_sentence_emb, top_span_mention_scores, c):
    """
    计算mention pairs的得分
    ...
    fast_antecedent_scores += self.get_fast_antecedent_scores(top_span_emb, top_sentence_emb) # [k, k]
  ...
  def get_predictions_and_loss(self, input_ids, char_ids, input_mask, text_len, speaker_ids, postag_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    postag_emb = tf.gather(tf.get_variable("postag_embeddings", [len(self.postag), self.config["feature_size"]],
                    initializer=tf.truncated_normal_initializer(stddev=0.02)), postag_ids)

    self.dropout = self.get_dropout(self.config["dropout_rate"], is_training)

    # [batch*seq_len, char_dim]
    mention_doc = self.flatten_emb_by_sentence(mention_doc, input_mask)
    postag_doc = self.flatten_emb_by_sentence(postag_emb, input_mask)
    mention_doc = tf.concat([mention_doc, postag_doc], axis=1)
    ...
    # TODO:构建句子级特征矩阵，用于向量拼接或查询
    sentence_map_min = tf.minimum(num_words//5, 300)           # 最多包含句子个数
    mask_sentence = tf.tile(tf.expand_dims(tf.range(sentence_map_min), 1), [1, num_words])
    sentence_map_expand = tf.tile(tf.expand_dims(sentence_map, 0), [sentence_map_min, 1])
    mask_sentence_emb = tf.equal(sentence_map_expand, mask_sentence)
    mask_sentence_emb = tf.expand_dims(mask_sentence_emb, 2)


    # for i in range(max0+1):
      # mask = np.ones(len(flattened_sentence_indices))
      # mask *= i
      # mask = tf.equal(np.array(flattened_sentence_indices, dtype=np.int), mask)
    mention_doc_expand = tf.tile(tf.expand_dims(mention_doc, 0), [sentence_map_min, 1, 1])
    # 对句子特征进行提取
    sentence_a = tf.to_float(mention_doc_expand) + tf.log(tf.to_float(mask_sentence_emb))
    emb_matrix = tf.where(sentence_a<0, tf.zeros_like(sentence_a), sentence_a)
    sentence_embs = tf.reduce_mean(emb_matrix, axis=1)

    # sentence_embs = tf.get_variable("sentence_embs", initializer=sentence_embs, dtype=tf.float32)
    ...
    # TODO:添加句子向量，融合句子级特征
    top_sentence_indices = tf.gather(candidate_sentence_indices, top_span_indices)  # [k]
    top_sentence_emb = tf.gather(sentence_embs, top_sentence_indices)
    # 字向量与句子向量拼接
    # top_span_emb = tf.concat([top_span_emb, top_sentence_emb], axis=1)
    # top_span_emb = top_span_emb + top_sentence_emb
    ...
    top_antecedents, top_antecedents_mask, top_fast_antecedent_scores, top_antecedent_offsets =\
      self.coarse_to_fine_pruning(top_span_emb, top_sentence_emb, top_span_mention_scores, c)
  ...
  def get_fast_antecedent_scores(self, top_span_emb, top_sentence_emb):
    """添加句子信息，sentenc_emb避免mention_paire的弊端"""
    top_span_emb = tf.concat([top_span_emb, top_sentence_emb], axis=1)
