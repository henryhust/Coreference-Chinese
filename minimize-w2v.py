...
def split_into_segments(document_state, max_segment_len, constraints1, constraints2):
  ...
  while current < len(document_state.subtokens):    # 字、以及英文词元素
    end = min(current + max_segment_len - 1, len(document_state.subtokens) - 1)
...
def get_document(document_lines, language, segment_len):
    """
    ...
    for line in document_lines[1]:
      ...
      if not sentence_end:
        ...
        subtokens = [char for char in word]                                        # bert处理, 英文被处理为词的元组，中文被切分成单个字
...
def minimize_partition(name, language, extension, labels, stats, seg_len, input_dir, output_dir):
    ...
    output_path = "{}/{}-word2vec.{}.{}.jsonlines".format(output_dir, name, language, seg_len)
    ...
    with open(output_path, "w", encoding="utf8") as output_file:
      for document_lines in documents:
        if skip(document_lines[0]):
          continue
        # try:
        document = get_document(document_lines, language, seg_len)     # 将数据写入文件
...
def minimize_language(language, labels, stats, seg_len, input_dir, output_dir, do_lower_case):
  ...
  minimize_partition("train", language, "v4_gold_conll", labels, stats, seg_len, input_dir, output_dir)
  minimize_partition("dev", language, "v4_gold_conll", labels, stats, seg_len, input_dir, output_dir)
  minimize_partition("test", language, "v4_gold_conll", labels, stats, seg_len, input_dir, output_dir)


if __name__ == "__main__":
  """
  命令行示例
  python minimize.py conll-2012/ontoNote5.0/ conll-2012/ontoNote5.0/  false
  """
  input_dir = sys.argv[1]       # 输入数据地址
  output_dir = sys.argv[2]      # 输出数据地址
  ...
  for seg_len in [128, 256, 384, 512]:
    ...
    minimize_language("chinese", labels, stats, seg_len, input_dir, output_dir, do_lower_case)
