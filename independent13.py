...
class CorefModel(object):
  ...
  def tensorize_example(self, example, is_training):
    ...
    example_tensors = (input_ids, segments_ids, segments_len, input_mask,  text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map)
  ...
  def truncate_example(self, input_ids, segments_ids, segments_len, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, sentence_offset=None):
    """对句子个数进行限制, [bert_base]max_training_sentences = 11,对segments_ids, segments_len不进行裁剪"""
    ...
    return input_ids, segments_ids, segments_len, input_mask, text_len, speaker_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map
  ...
  def get_predictions_and_loss(self, input_ids, segments_ids, segments_len, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    # TODO:构建句子级Bigru+attention特征向量
    flattened_sentence_indices = sentence_map                                   # 字符所属句子
    segments_char_emb = tf.gather(tf.get_variable("char_lstm_embedding", [len(self.char_dict), self.config["feature_size"]],
                    initializer=tf.truncated_normal_initializer(stddev=0.02)), segments_ids)

    bilstm_out = util.biGRU_layer(segments_char_emb, 20, segments_len)
    sentence_emb = util.attention(bilstm_out, 20)
    # sentence_emb = bilstm_out[:, -1, :]
    ...
    # top_sentence_emb = util.biLSTM_layer(model_inputs=top_sentence_emb, lstm_dim=, lengths=, name=None)
