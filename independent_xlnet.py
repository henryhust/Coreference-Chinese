...
from xlnet import xlnet, optimization, model_utils
...
class CorefModel(object):
  def __init__(self, config):
    ...
    self.xlnet_config = xlnet.XLNetConfig(json_path=config["xlnet_config_file"])
    self.run_config = xlnet.create_run_config(is_training=False, is_finetune=True, config=config)
    self.tokenizer = Tokenizer(self.config["vocab_file"])
    ...
    input_props.append((tf.int32, [None, None]))    # segments
    input_props.append((tf.int32, [None, 1]))    # lenths
    ...
    assignment_map, initialized_variable_names = model_utils.get_assignment_map_from_checkpoint(tvars, config['tf_checkpoint'])
    ...
    self.train_op = optimization.create_xlnet_optimizer(tvars, self.loss, self.config, self.global_step, num_train_steps, num_warmup_steps, False, freeze=-1)
  ...
  def tensorize_example(self, example, is_training):
    ...
    input_ids, input_mask, speaker_ids, segment_ids = [], [], [], []

    # 将输入数据特征化，转化为ids_input
    for i, (sentence, speaker) in enumerate(zip(sentences, speakers)):
        sentence_str = "".join(sentence[:len(sentence)-1])
        encoded = self.tokenizer.encode(sentence_str)

        # Padding
        sent_input_ids = [0]*(max_sentence_length-1-len(encoded)) + encoded + [self.tokenizer.SYM_CLS]
        # sent_input_ids = self.tokenizer.convert_tokens_to_ids(sentence)
        sent_seg_ids = [3]*(max_sentence_length-len(sentence))+[0]*(len(sentence)-1)+[1]        # pad + sentence + CLS
        sent_input_mask = [1]*(max_sentence_length-len(sentence)) + [0] * len(sentence)
        sent_speaker_ids = [speaker_dict.get(s, 3) for s in speaker]
        sent_speaker_ids = [0] * (max_sentence_length-len(sent_speaker_ids)) + sent_speaker_ids

        # while len(sent_input_ids) < max_sentence_length:
        #     """padding"""
        #     sent_input_mask = [0] + sent_input_mask
        #     sent_speaker_ids = [0] + sent_speaker_ids

        input_ids.append(sent_input_ids)
        speaker_ids.append(sent_speaker_ids)
        segment_ids.append(sent_seg_ids)
    ...
    memory_lengths = np.ones_like(input_ids[:, :1])
    assert num_words ==(len(sentences)*max_sentence_length-np.sum(input_mask)), (num_words,len(sentences)*max_sentence_length-np.sum(input_mask))
    ...
    example_tensors = (input_ids, input_mask,  text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, segment_ids, memory_lengths)
  ...
  def truncate_example(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, segments, memory_lengths, sentence_offset=None):
    ...
    segments = segments[sentence_offset:sentence_offset + max_training_sentences, :]
    memory_lengths = memory_lengths[sentence_offset:sentence_offset + max_training_sentences, :]
    ...
    return input_ids, input_mask, text_len, speaker_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map, segments, memory_lengths
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, segments, memory_lengths):
    xlnet_input_ids = tf.transpose(input_ids, [1, 0])
    xlnet_segments_ids = tf.transpose(segments, [1, 0])
    xlnet_input_mask = tf.to_float(tf.transpose(input_mask, [1, 0]))

    xlnet_model = xlnet.XLNetModel(xlnet_config=self.xlnet_config, run_config=self.run_config,
                                   input_ids=xlnet_input_ids, seg_ids=xlnet_segments_ids, input_mask=xlnet_input_mask)
    # The feature for each token, with shape (batch_size, target_len, units).
    xlnet_output = xlnet_model.get_sequence_output()
    xlnet_output = tf.transpose(xlnet_output, [1, 0, 2])

    self.mention_doc = xlnet_output
    self.dropout = self.get_dropout(self.config["dropout_rate"], is_training)
    
    # input_mask转换
    tmp_mask = np.ones_like(input_mask)
    input_mask = tmp_mask - tf.to_int64(input_mask)

    mention_doc = self.flatten_emb_by_sentence(xlnet_output, input_mask)    # 掩掉padding，形成【num_seq * length_seq, dim】
  ...
  def evaluate(self, session, global_step=None, official_stdout=False, mod="eval", keys=None, eval_mode=False):
    ...
    print("Mention Recall = {:.2f}%".format(mention_pre / mention_gold * 100))
    ...
    return util.make_summary(summary_dict), f, p, r
